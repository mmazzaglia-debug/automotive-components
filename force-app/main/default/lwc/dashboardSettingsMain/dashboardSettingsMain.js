...
export default class DashboardSettingsMain extends LightningElement {
  ...
  EMPTY_CARD = {
    ...
    layout: 'SIDE',
    subFilters: [
      { key: '', field: '', operator: '', value: '', color: '#747474', label: '', icon: '' },
    ],
  };
  ...
  async connectedCallback() {
    ...
    this.objectsOptions = allObjects?.sort((f1, f2) => (f1.label < f2.label ? -1 : 1)) ?? [];
  }
  ...
  GetAllDashboardSettings({ data, errors }) {
    if (data) {
      const dbDashboardSettings = data?.uiapi?.query['Mobile_Dashboard_Setting__c']?.edges;
  ...
  @api duplicateCard = async (ind) => {
    try {
      ...
      const Id = await insertNewCard({ data: this.buildDataForServer(newCard) });
  ...
  deleteCard = async (ind) => {
    const toDelete = await AlertMessage.open({
      size: 'small',
      modalTitle: this.LABELS.MobileDashboard_settings_delete_card_warning_title,
  ...
  openEditCardModal = async (isEdit, cardIndex) => {
    const result = await EditCardModal.open({
      ...
      card: this.dashboardCards[cardIndex] ?? { ...this.EMPTY_CARD, key: this.generateUniqueID() },
  ...
  async getAllObjLabels(dbDashboardSettings) {
    const allApiNames = dbDashboardSettings?.map((s) => s.node.Object_Name__c.value);
