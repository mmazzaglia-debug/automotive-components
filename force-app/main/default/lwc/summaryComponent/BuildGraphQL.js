export class DQM {
  ...
  static sanitizex(s = '') {
    return (s ?? '').replaceAll('and', '&').replaceAll('or', '|').replaceAll(' ', '').toLowerCase();
  }
  ...
  static assembleQuery(obx, ret, main_filter) {
    try {
      ...
      for (let i = 1; i <= max_allowed_digit; i++) {
        ...
        finalx = finalx.replaceAll(`<<SALESFORCE_${i}>>`, DQM.expression_maker(filter));
      }
  ...
  static initialPhraseValidation(phrase = '') {
    try {
      ...
      if (!s || s.includes('&&') || s.includes('||') || s.includes('()')) return '031';
  ...
  static mainFilterExpressionMaker(obj) {
    try {
      ...
      let main_filter_logic = obj?.node?.Main_Filter_Logic__c?.value?.toLowerCase().trim() ?? '';
      ...
      if (main_filter_logic == 'custom') {
        ...
        if (!oQry.is_success) {
          DQM.cpx(`${oQry.error_msg} - custom phrase:[${phrase}] - Error ${oQry.error_code}`);
          return omni_main_filter;
        }
      ...
      main_filter_array.forEach((filter) => {
        ...
        if (['today', 'last', 'next'].includes(operator)) {
          ...
          let valuex = DQM.valueFormatter(1, filter?.value, filter?.fieldType, operator == 'like');
  ...
  static expression_maker(filter) {
    try {
      ...
      if (['today', 'last', 'next'].includes(operator)) {
        ...
        let valuex = DQM.valueFormatter(2, filter?.value, filter?.fieldType, operator == 'like');
  ...
  static gqlQueryMaker(obj) {
    try {
      ...
      let sub_filter_expressions_array = DQM.subFilterExpressionsArrayMaker(obj);
